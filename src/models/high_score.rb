...
class High_score
  ...
  def self.token_topfive?(name, total_tokens, streak)
    # read file
    File.open("models/token_topfive.txt", "r") do |file|
      ...
      if @top_token[4][1].to_i < total_tokens.to_i
        ...
        File.open("models/token_topfive.txt", "w") do |file|
          file.puts(@top_token)
        end
      end
    end
  ...
  def self.streak_topfive?(name, total_tokens, streak)
    # read file
    File.open("models/streak_topfive.txt", "r") do |file|
      f = file.readlines.map(&:chomp)
      @ary2 = []
      # put into @ary2 array
      f.each_index do |index|
      ...
      if @top_streak[4][1].to_i < streak.to_i
        ...
        File.open("models/streak_topfive.txt", "w") do |file|
